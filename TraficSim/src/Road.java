...
public class Road{
  ...
  public void paint(Graphics g){
    //Paint all the childRoads
    for(Road r:children)
      r.paint(g);
    
    int length = this.length*20;
    
    //Paint the stopLights
    if(tLight != null)
      tLight.paint(g, posX+(dirX*length), posY+(dirY*length), 20);
    
    //Set the color to the roads pink color
    ...
    //Set the step boolean to false and thereby making it able to step once more
    steped = false;
  }
}
